...
void parse_printf_fmt(const std::string &fmt, std::string &flags, std::string &width, std::string &prec, std::string &len, std::string &spec)
{
	...
	// TODO this check should be done for all situations - implement and check!
	if (fmt.find_first_of("n*%") != std::string::npos)		// these symbols are not currently handled
		throw EObjFunc(stringFormatArr("Format '{0:%s}' is not acceptable, symbols n*% are not currently handled", fmt));
...
{						// If 'width' > 0 and 'fmt' does not specify the width, then the 'width' provided is pushed to 'fmt', and '-' flag is added.
	if (width > 0) {	// Otherwise 'fmt' is intact, and 'width' = 0 is set for output.
		std::string flg, wdth, prc, ln, spc;

		assert(fmt.size() > 0 && fmt[0] == '%');
