...
public:
	...
	virtual std::string ToString(std::string fmt = "", size_t &width = 0) const;
};
//------------------------------------------------------------------------------------------
...
std::string stringTagPrintf(const std::string &input_text, const std::map<std::string, ValBase*> &tag_val, int &count, std::set<std::string> &tags_left);
			// Writes values corresponding to 'tags' in "input_text". The 'tag' locations may be of the form $tag, $tag%fmt (e.g. format %fmt = %20.16g).	TODO re-read
			...
			// the format will be adjusted to preserve the column with, possibly by consuming some of the spaces.
			...
			// Parameter names encountered in "input_text" are removed from "tags_left" set.
