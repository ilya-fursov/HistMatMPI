...
//			   	   	      0.00344730956393175, 0.00126885916873903, 0.000466968178674323, 0.000171839237422763, 6.32310794199025E-05};
const double eps_tab[] = {0.100000000082529, 0.0204510680740604, 0.00488141404657189, 0.00125479499796607, 0.000337619989687407,
			     9.37692207141492E-05, 2.66673094905714E-05, 7.72603733441693E-06, 2.27231472991232E-06, 6.76732145131486E-07,
			     2.03695180886576E-07, 6.18766998423662E-08, 1.89476554526184E-08, 5.84335176557212E-09, 1.81349145464859E-09,
			     5.66030845742479E-10, 1.7758375498998E-10, 5.59766236359026E-11, 1.77206602154294E-11, 5.63215708126606E-12,
			     1.79664294069776E-12, 5.75078200485321E-13, 1.84658413908786E-13, 5.94700533747974E-14, 1.92058686673218E-14,
			     6.21875476264939E-15, 2.018557840435E-15, 6.56731403023357E-16, 2.14135678259466E-16, 6.99673576666644E-17,
			     2.29066593977603E-17, 7.51356778623385E-18, 2.46893926522144E-18, 8.12678646847577E-19, 2.67940675375519E-19,
		   	     8.84788091211583E-20, 2.92612113204257E-20, 9.69105939338633E-21, 3.21404082986298E-21, 1.06735662211889E-21,
			     3.54914732896517E-22, 1.18161078426918E-22, 3.9385978388295E-23};
...
cmaes_SayHello(cmaes_t *t)
{
  ...
  sprintf(t->sOutString,
          "(%d,%d)-CMA-ES(mu_eff=%.1f), Ver=\"%s\", dimension=%d, diagonalIterations=%ld, randomSeed=%d (%s), eps=%g",
          t->sp.mu, t->sp.lambda, t->sp.mueff, t->version, t->sp.N, (long)t->sp.diagonalCov,
          t->sp.seed, getTimeStr(), t->eps_i);
                ...
                const char *input_parameter_filename)
{
  ...
  t->BiBj = (double**)new_void(N, sizeof(double*));	// 19.04.2013
  ...
  for (i = 0; i < N; ++i) {
      ...
      t->B[i] = new_double(N);
      t->BiBj[i] = new_double(N);					// 19.04.2013
    }
  ...
  int _k = N/2;								// 16.06.2013
  if (_k < 1)
	  _k = 1;
  if (_k <= 43)
	  t->eps_i = eps_tab[_k-1];
  else
	  t->eps_i = exp(log(eps_tab[42]) - log(3.0)*(_k - 43));

  t->tot_models = 0;						// 15.06.2013
  t->tot_ignored = 0;						// 15.06.2013
  t->dens_val = new_double(t->sp.lambda);			// 19.04.2013
  t->x_mu = new_double(N);							// 19.04.2013
  for (i = 0; i < t->sp.lambda; ++i) {
...
cmaes_exit(cmaes_t *t)
{
  ...
  free( t->C);
  free( t->B);
  free( t->BiBj);			// 19.04.2013
  free( t->dens_val);		// 19.04.2013
  free( t->x_mu);			// 19.04.2013
  free( t->index);
...
cmaes_SamplePopulation(cmaes_t *t)
{
  ...
  TestMinStdDevs(t);
      ...
      for (i = 0; i < N; i++)
      {
    	  for (j = 0; j < N; j++)
    	  {
    		  ...
    		  t->BiBj[i][j] = v;
    		  if (i == j && fabs(t->BiBj[i][j]-1) > 1e-14)
    			  printf("B[%d, %d]\t%.16g\n", i, j, t->BiBj[i][j]);
    		  if (i != j && fabs(t->BiBj[i][j]) > 1e-14)
    			  printf("B[%d, %d]\t%.16g\n", i, j, t->BiBj[i][j]);
    	  }
      }
#endif
  ...
  for (iNk = 0; iNk < t->sp.lambda; ++iNk)
  {
      ...
      for (i = 0; i < N; i++)
      {
    	  t->x_mu[i] =  t->rgrgx[iNk][i] - xmean[i];
    	  detD *= t->rgD[i];
      }
      ...
      for (i = 0; i < N; i++)
    	  for (j = 0; j < N; j++)
    		  for (int k = 0; k < N; k++)
    			  exp_sum += t->x_mu[i] * t->B[i][k] / (t->rgD[k]*t->rgD[k]) * t->B[j][k] * t->x_mu[j];

      double exp_val = exp(-0.5 / (t->sigma*t->sigma) * exp_sum);
      if (exp_val < t->eps_i)		// 16.06.2013
      {
    	  t->dens_val[iNk] = 0;
    	  t->tot_ignored++;
      }
      else
    	  t->dens_val[iNk] = exp_val / (pow(2*PI, double(N)/2) * t->sigma * fabs(detD));
...
cmaes_ReSampleSingle( cmaes_t *t, int iindex)
{
  ...
  for (i = 0; i < N; i++)
  {
	  t->x_mu[i] =  t->rgrgx[iindex][i] - t->rgxmean[i];
	  detD *= t->rgD[i];
  }
  ...
  if (exp_val < t->eps_i)			// 16.06.2013
  {
	  ...
	  t->dens_val[iindex] = exp_val / (pow(2*PI, double(N)/2) * t->sigma * fabs(detD));
                         ...
                         const char *appendwrite)
{
  ...
  MPI_Bcast(&error, 1, MPI_BYTE, 0, MPI_COMM_WORLD);

  if (error)	// error is sync
  {
 ...
 */
{
  ...
  char const *keyend;	//, *keystart;	31.03.2013
  const char *s = "few";
  if (key == NULL)
    key = s;
  ...
  while (key < keyend)
    {
      ...
      if (strncmp(key, "C", 1) == 0)
        {
          ...
          for (i = 0; i < N; ++i)
            for (j = 0; j <= i; ++j)
              fprintf(fp, "%.12g%c", t->C[i][j], (j==i)?'\n':'\t');
          ++key;
        }
      /* (processor) time (used) since begin of execution */
      if (strncmp(key, "clock", 4) == 0)
        {
          timings_update(&t->eigenTimings);								// TODO: possible MPI deadlock!
      ...
      if (strncmp(key, "resume", 6) == 0)
        {
          ...
          for(i=0; i<N; ++i)
            fprintf(fp, "%.12g%s", t->rgps[i], (i==N-1) ? "\n":"\t");
          fprintf(fp, "path for C\n");
          for(i=0; i<N; ++i)
            fprintf(fp, "%.12g%s", t->rgpc[i], (i==N-1) ? "\n":"\t");
          fprintf(fp, "sigma %.12g\n", t->sigma);
      ...
      if (strncmp(key, "xmean", 5) == 0) {
        for(i=0; i<N; ++i)
          fprintf(fp, "%s%.12g", (i==0) ? "":"\t", t->rgxmean[i]);
        while (*key != '+' && *key != '\0' && key < keyend)
          ++key;
        fprintf(fp, "%c", (*key=='+') ? '\t':'\n');
      }
      ...
      if (*key == '\0')
        ...
        ERRORMESSAGE("cmaes_t:WriteToFilePtr(): unrecognized key '", key, "'", 0);		// TODO possible MPI deadlock
        while (*key != '+' && *key != '\0' && key < keyend)
          ++key;
      }
      while (*key == '+')
...
timings_start(timings_t *t) {
  ...
  t->lasttictoctime = 0;
  t->istic = 0;
  t->lastclock = clock();		// clock_t = unsigned long
...
{
  clock_t cloc = clock();
  MPI_Bcast(&cloc, 1, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
            ...
            char const *s4)
{
  ...
  fflush(stdout);
  throw HMMPI::Exception("(eng)", "CMA-ES aborted");
}
                   ...
                   char const *s3, char const *s4)
{
  ...
  */
  ...
  if (error)
  {
      ...
      throw HMMPI::Exception("CMA-ES aborted");
  }
...
double **ReadLimits(const char *fname, int dim)
{
	...
	i = 0;
    while ((fgets(s, sizeof(s), fp) != NULL)&&(i < dim))
    {
		sscanf(s, "%lg%lg", &(res[i][0]), &(res[i][1]));
		i++;
	}
...
int is_feasible(double *par, double **lim, int dim)
{
	...
	for (i = 0; i < dim; i++)
	{
		if ((par[i] < lim[i][0])||(par[i] > lim[i][1]))
			return 0;
	}
